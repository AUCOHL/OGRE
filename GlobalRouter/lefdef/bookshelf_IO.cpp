...
void createHash(char benchmarkPath[], char nodesFile[]) 
{
    ...
    if(NodesInfo == NULL) {
		...
		if (!NodesInfo)
		{
			char x_temp[] = "bookshelf_IO: allocation failure in createNodesHash()";
			// runtimeError(x_temp);
		}
    } else {
		char x_temp[] = "bookshelf_IO: NodesHash is not NULL";
        // runtimeError(x_temp);
    }
    ...
    if(RN == NULL && hashFlag == NULL) {
		...
		char x_temp[] = "bookshelf_IO: error while creating input hash";
		// runtimeError(x_temp);
    }
    ...
    if(cellName == NULL) {
		...
		if (!cellName)
		{
			char x_temp[] = "bookshelf_IO: allocation failure for cellName";
			// runtimeError(x_temp);
		}
...
//    printf("Reading %s ...\n", nodesFile);
    ...
    if(cellWidth == NULL && cellHeight == NULL && cellArea == NULL) {
		...
		char x_temp[] = "bookshelf_IO: allocation error in readNodesFile()";
        // runtimeError(x_temp);
    }
...
void readNetsFile(char benchmarkPath[], char netsFile[])
{
    ...
    if(netlist == NULL && xPinOffset == NULL && yPinOffset == NULL &&
       netlistIndex == NULL) {
		...
		char x_temp[] = "bookshelf_IO: allocation error in readNetsFile()";
       // // // runtimeError(x_temp);
    }
...
//    printf("Reading %s ...\n", plFile);
    ...
    if(xCellCoord == NULL && yCellCoord == NULL && cellOrientation == NULL) {
		...
		char x_temp[] = "bookshelf_IO: allocation error in readPlFile()";
        // // runtimeError(x_temp);
    }
...
void readSclFile(char benchmarkPath[], char sclFile[])
{
	...
	if (!core_row)
	{
		char x_temp[] = "bookshelf_IO: allocation failure in createRowStructure()";
		// runtimeError(x_temp);
	}
    if((fp=fopen(file_name, "r"))==NULL) {
        sprintf(error_text, "bookshelf_IO: Cannot open %s file", file_name);
...
//    printf("Reading %s ...\n", sclFile);
                ...
                *line = '\0';
                ...
                if(strcmp(temp, "Coordinate")==0) {
                    ...
                    if(subrow_count > 99) {
						...
						char x_temp[] = "bookshelf_IO: threshold reached for #subrows per row";
                        // runtimeError(x_temp);
                    }
    ...
    if(rowBlockageCount > 0) {
        ...
        if(numRowBlockages != rowBlockageCount) {
			char x_temp[] = "bookshelf_IO: error during createRowBlockage()";
            // runtimeError(x_temp);
        }
    }
...
void readWtsFile(char benchmarkPath[], char wtsFile[], int net_weights)
{
    ...
    if(netWeight == NULL) {
		...
		char x_temp[] = "bookshelf_IO: allocation error in readWtsFile()";
        // runtimeError(x_temp);
    }
    ...
    if(net_weights) {
        ...
        if(NetsInfo == NULL) {
			...
			if (!NetsInfo)
			{
				char x_temp[] = "bookshelf_IO: allocation failure in createNetsHash()";
				// runtimeError(x_temp);
			}
        } else {
			char x_temp[] = "bookshelf_IO: NetsHash is not NULL";
            // runtimeError(x_temp);
        }
        for(j=0; j<hashSize; j++)
            NetsInfo[j] = NULL;
        ...
        if(RN == NULL && hashFlag == NULL) {
			...
			char x_temp[] = "bookshelf_IO: error while creating nets hash";
            // runtimeError(x_temp);
        }
