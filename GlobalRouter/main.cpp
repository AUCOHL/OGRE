...
void Route(bool failed,string name,int threadsCounter,int&bufferId)
{
	bufferId = 0;
	int counter = 0;
	for (auto route : route_nodes)
	{
		cout<<counter<<",";
		MapSearchNode start;
		start.z = route.first.z;
		start.x = route.first.x;
		start.y = route.first.y;
		MapSearchNode goal;
		goal.z = route.second.z;
		goal.x = route.second.x;
		goal.y = route.second.y;
		cout<<name<<endl;
		//
		...
		failed = false;
		//tp.enqueue(routeTwoPoints, start, goal, bufferId, name, failed);
		cout<<"def here"<<endl;
		...
		if (failed)
		{
			...
			failed = false;
			Route(failed,name,threadsCounter,bufferId);
			return;
		}
...
int main(int argc, char *argv[])
{
	...
	while(ordered_nets.size())
    {
		...
		if (canRun)
		{
			...
			// }
			name = netName.second;
			Route(false,name,threadsCounter,bufferId);
		}
		else
		{
